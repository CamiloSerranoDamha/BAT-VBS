...
SUBROUTINE BAT(M_org, O2C_org, H2C_org, N2C_org, Group_org, x_org, y_water, y_org)
	...
	! inputs
    INTEGER(4), INTENT(IN) :: Group_org                    ! Type of oxygen-bearing functional group that represents the organic species
    REAL(8), INTENT(IN)    :: O2C_org, H2C_org, N2C_org, & ! Elemental ratios (O/C, H/C, N/C) of the organic species in the binary mixture  
                                & M_org, x_org                ! Molecular weight (g/mol) and mole fraction of the organic species in the binary mixture
	! outputs
    REAL(8), INTENT(OUT)   :: y_water, y_org               ! Activity cofficient of water and organic species in the binary mixture
	...
	CALL BAT_SIMULATION(M_org, O2C_org, H2C_org, N2C_org, Group_org, x_org, y_water, y_org)
...
SUBROUTINE BAT_INVERSION(N_org, M_org_j, O2C_org_j, H2C_org_j, N2C_org_j, Group_org_j, C_PM_org_j, a_water, &
	...
	!INTEGER, PARAMETER  						:: rp = KIND(1.0D0)                 			! Precision of REALS
    ! Inputs
    INTEGER(4), INTENT(IN)                     :: N_org                                     	! Number of organic species in the system
    INTEGER(4), DIMENSION(N_org), INTENT(IN)   :: Group_org_j                                  ! Type of oxygen-bearing functional group of each organic species j in the system
    REAL(8), DIMENSION(N_org), INTENT(IN)      :: M_org_j, O2C_org_j, H2C_org_j, N2C_org_j, &  ! Molecular weight (g/mol) and elemental ratios (O/C, H/C, N/C) of each organic species j in the system
													& C_PM_org_j                                ! Total "dry" mass concentration of each organic species j in the particle (ug/m3)
    REAL(8), INTENT(IN)                        :: a_water                                      ! Target relative humidity (gas-phase water activity) that the mole fraction of water (and organics) in the mixture must match (scale of 0 to 1)

    ! Outputs
    REAL(8), INTENT(OUT)                       :: C_PM_water_org_alpha, C_PM_water_org_beta    ! Total (organic + water) mass concentration in liquid phases alpha and beta (ug/m3)
    REAL(8), DIMENSION(N_org), INTENT(OUT)     :: y_org_j_alpha, y_org_j_beta, &               ! Activity coefficient of organic species j in liquid phases alpha and beta
	...
	CALL BAT_INVERSION_SIMULATION(N_org, M_org_j, O2C_org_j, H2C_org_j, N2C_org_j, Group_org_j, &
		& C_PM_org_j, a_water, y_org_j_alpha, y_org_j_beta, w_org_j_alpha, w_org_j_beta, &
		& C_PM_water_org_alpha, C_PM_water_org_beta)
...
SUBROUTINE BAT_INVERSION_VBS(N_org, M_org_j, O2C_org_j, H2C_org_j, N2C_org_j, C_sat_org_j, C_gas_particle_org_j, &
	...
	!INTEGER, PARAMETER  						:: rp = KIND(1.0D0)                 			! Precision of REALS
    ! inputs
	!INTEGER(wip), INTENT(IN)					 :: N_a_water									! Number of water activities 
    INTEGER(4), INTENT(IN)                     :: N_org                                    	! Number of organic species j in the system
    INTEGER(4), DIMENSION(N_org), INTENT(IN)   :: Group_org_j									! Type of oxygen-bearing functional group of each organic species j in the system
    REAL(8), DIMENSION(N_org), INTENT(IN)      :: M_org_j, O2C_org_j, H2C_org_j, N2C_org_j, &	! Molar mass (g/mol) and elemental ratios (O/C, H/C) of each organic species j in the system
	...
	REAL(8), INTENT(IN)	 :: a_water												
    !REAL(wrp), DIMENSION(N_a_water), INTENT(IN)	 :: a_water                                  	! Target relative humidity (gas-phase water activity) that the mole fraction of water (and organics) in the mixture must match

    ! output
	REAL(8), INTENT(OUT)			             :: C_PM_water_alpha, C_PM_water_beta
    !REAL(wrp), DIMENSION(N_a_water), INTENT(OUT)			:: C_PM_water_alpha, C_PM_water_beta      		! Equilibrium water mass concentration in liquid phases alpha and beta that results from VBS calculations (ug/m3)
	REAL(8), DIMENSION(N_org), INTENT(OUT)     :: C_PM_org_j_alpha, C_PM_org_j_beta
    !REAL(wrp), DIMENSION(N_a_water, N_org), INTENT(OUT)     :: C_PM_org_j_alpha, C_PM_org_j_beta      		! Equilibrium mass concentration of each organic species j in liquid phases alpha and beta that results from VBS calculations (ug/m3)
        		 
    ! ! variables used to map precision of REALS
    ! REAL(wrp)                                   :: C_PM_water_alpha_wrp, C_PM_water_beta_wrp, &
													! & a_water_wrp
    ! REAL(wrp), DIMENSION(N_org)                 :: O2C_org_j_wrp, H2C_org_j_wrp, N2C_org_j_wrp, &
													! & M_org_j_wrp, C_gas_particle_org_j_wrp, &
													! & C_sat_org_j_wrp
	! REAL(wrp), DIMENSION(N_org)				   ::	C_PM_org_j_alpha_wrp, C_PM_org_j_beta_wrp
    ...
    CALL BAT_INVERSION_VBS_SIMULATION(N_org, M_org_j, O2C_org_j, H2C_org_j, N2C_org_j, &
		& C_sat_org_j, C_gas_particle_org_j, Group_org_j, a_water, C_PM_org_j_alpha, &
		& C_PM_org_j_beta, C_PM_water_alpha, C_PM_water_beta)
...
SUBROUTINE CSAT_APPROX(N_org, M_org_j, O2C_org_j, C_star_org_j, C_gas_particle_org_j, Group_org_j, C_sat_org_j)
    ...
    INTEGER(4), INTENT(IN)                     :: N_org                                    	! Number of organic species j in the system
    INTEGER(4), DIMENSION(N_org), INTENT(IN)   :: Group_org_j									! Type of oxygen-bearing functional group of each organic species j in the system
    REAL(8), DIMENSION(N_org), INTENT(IN)      :: M_org_j, O2C_org_j, & 	                    ! Molar mass (g/mol) and O/C elemental ratio of each organic species j in the system
													& C_gas_particle_org_j, &                	! Mass concentration of each organic species j in gas phase + particle phase (ug/m3)
                                                    & C_star_org_j                            	! Effective concentration C* of each organic species j at the temperature of interest (ug/m3)


    ! output
    REAL(8), DIMENSION(N_org), INTENT(OUT)		:: C_sat_org_j                            	    ! Pure component saturation mass concentration of each organic species j at the temperature of interest (ug/m3)
...
SUBROUTINE DENSITY_ORG_ESTIMATE(N_org, M_org_j, O2C_org_j, H2C_org_j, N2C_org_j, density_org_j)
	...
	! inputs
	INTEGER(4), INTENT(IN) 					:: N_org										! Number of organic species j in the system
	REAL(8), DIMENSION(N_org), INTENT(IN)  	:: M_org_j, O2C_org_j, H2C_org_j, N2C_org_j 	! Elemental ratios (O/C, H/C, N/C)
																								! and molecular weight of each org species j

	! outputs
	REAL(8), DIMENSION(N_org), INTENT(OUT)		:: density_org_j       							! Density estimate of each org species j
	...
	CALL Org_density_Estimate_KGv1(M_org_j, O2C_org_j, H2C_org_j, N2C_org_j, density_org_j)
